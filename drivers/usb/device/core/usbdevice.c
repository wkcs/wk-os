                                     ...
                                     uint8_t number_of_proerty)
{
    ...
    if(data == NULL)
    {
        ...
        data = (uint8_t *)wk_alloc(header.dwLength, 0, 0);
        WK_ERROR(data != NULL);
        pdata = data;
...
{
#ifndef USB_DEVICE_COMPOSITE
    if(!list_empty(&class_list))
    {
        pr_info("[D/USBD] If you want to use usb composite device please define USB_DEVICE_COMPOSITE\r\n");
        return -1;
    }
#endif
